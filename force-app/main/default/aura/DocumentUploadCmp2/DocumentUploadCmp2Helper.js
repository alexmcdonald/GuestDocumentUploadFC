            ...
            function (response) {
                ...
                if (response && response.length > 0) {
                    ...
                    const totalScore = response.reduce(function (total, doc) {
                        return doc.uploadedDocuments.length > 0 ? total + doc.document.qsydApps_AO__Score__c : total;
                    }, 0);
                    const numMandatoryUploaded = response.reduce(function (total, doc) {
                        return (doc.document.qsydApps_AO__Mandatory__c === true && doc.uploadedDocuments.length === 0) ? total - 1 : total;
                    }, 0);
                    ...
                    const navigationEvent = $A.get("e.c:LightningFlowNavigationEvt2");
